...
import { CheckIcon, CloseIcon } from "@resonance/ui/icons";
import { UniformOption } from "./uniform-option";
import { SiteAddressAutocomplete } from "../site-address-autocomplete";
import { useEditSiteDrawer } from "../../hooks/useEditSiteDrawer";
...
export const EditSiteDrawer = ({ site, onClose, onSave }: EditSiteDrawerProps) => {
  const {
    formData,
    errors,
    isPending,
  ...
  return (
    <SideDrawer isOpen={!!site} onClose={onClose} width={460}>
      ...
      <Container as="form" onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
        ...
        <Container className="border-t border-border px-4 py-4 flex gap-2.5">
          <Button type="button" className="flex-1" variant="neutral" onClick={onClose}>
            Cancel
          </Button>
          <Button
            type="submit"
